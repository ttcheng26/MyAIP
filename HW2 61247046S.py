...
def show(): # 顯示照片
    ...
    while img_path != '':
        ...
        tk_img = ImageTk.PhotoImage(image=Image.fromarray(cv2.resize(cv_img, (512, 512))))
        canvas1.delete('all')
...
def histogram(): # 顯示直方圖
    ...
    tk_img = ImageTk.PhotoImage(image=Image.fromarray(cv2.resize(img2, (512, 512))))
    canvas2.delete('all')
    canvas2.create_image(0,0, anchor='nw', image=tk_img)
    canvas2.tk_img = tk_img
    os.remove("hist.png")
...
btn5.place(x=700, y=0) # histogram
