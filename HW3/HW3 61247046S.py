...
def show():  # 顯示照片
    ...
    while img_path != '':
        ...
        tk_img = ImageTk.PhotoImage(
            image=Image.fromarray(cv2.resize(cv_img, (320, 320))))
        canvas1.delete('all')
...
def gaussian():
    ...
    def execute_and_quit():
        ...
        tk_img = ImageTk.PhotoImage(image=Image.fromarray(
            cv2.resize(noisy_image, (320, 320))))
        tk_img2 = ImageTk.PhotoImage(image=Image.fromarray(
            cv2.resize(blank_image, (320, 320))))
        canvas2.delete('all')
        canvas3.delete('all')
    ...
    buttonExample = tk.Button(newWindow, text="產生高斯雜訊",
                              command=execute_and_quit)
...
def salt_and_pepper():
    ...
    def execute_and_quit2():
        ...
        impulse_coordinates = [np.random.randint(
            0, height, num_pixels), np.random.randint(0, width, num_pixels)]
    ...
    buttonExample2 = tk.Button(
        newWindow2, text="產生椒鹽雜訊", command=execute_and_quit2)
...
def histogram():  # 顯示直方圖
    ...
    tk_img = ImageTk.PhotoImage(
        image=Image.fromarray(cv2.resize(hist1, (320, 320))))
    canvas4.delete('all')
    ...
    tk_img2 = ImageTk.PhotoImage(
        image=Image.fromarray(cv2.resize(hist2, (320, 320))))
    canvas5.delete('all')
    ...
    tk_img3 = ImageTk.PhotoImage(
        image=Image.fromarray(cv2.resize(hist3, (320, 320))))
    canvas6.delete('all')
